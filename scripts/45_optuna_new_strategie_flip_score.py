...
def _compute_score(*, equity_end: float, equity_dd: float, winrate: float, dd_weight: float, winrate_weight: float) -> float:
    return float(equity_end) - float(dd_weight) * float(equity_dd) + float(winrate_weight) * float(winrate)
...
def main() -> int:
    ...
    if not isinstance(out_cfg, dict):
        out_cfg = {}
    out_dir = Path(str(out_cfg.get("out_dir") or "data/processed/backtests/optuna_new_strategie_flip_score"))
    ...
    study_name = str(optuna_cfg.get("study_name") or "new_strategie_flip_score_2024")
    ...
    if not isinstance(score_cfg, dict):
        score_cfg = {}
    dd_weight = float(score_cfg.get("dd_weight") or 1.0)
    winrate_weight = float(score_cfg.get("winrate_weight") or 0.5)
    ...
    def _evaluate_cfg_one(cfg, *, df_base: pd.DataFrame, interval: str) -> dict[str, float]:
        ...
        ts = ts.dropna()
        if ts.empty:
            return {"n_trades": 0.0, "equity_end": -1e6, "equity_dd": 1e6, "winrate": 0.0, "score": -1e6}
        ...
        score = _compute_score(
            ...
            winrate_weight=float(winrate_weight),
        )
    ...
    def _evaluate_cfg_robust(cfg) -> dict[str, float]:
        ...
        for (sym, itv, y), df in data_cache.items():
            ...
            scores.append(float(m.get("score") or 0.0))

        if not scores:
            return {"n_trades": 0.0, "equity_end": -1e6, "equity_dd": 1e6, "winrate": 0.0, "score": -1e6}

        if int(ntr_total) < int(min_trades):
            return {"n_trades": float(ntr_total), "equity_end": -1e6, "equity_dd": 1e6, "winrate": 0.0, "score": -1e6}
    ...
    def objective(trial):
        ...
        trial.set_user_attr("score", float(m["score"]))
    ...
    trials_df = pd.DataFrame(trials_rows).sort_values(["value"], ascending=False)
    trials_csv = out_dir / "optuna_trials.csv"
    trials_df.to_csv(trials_csv, index=False, float_format="%.10f")

    best_trial = study.best_trial if len(study.trials) else None
    ...
    if best_trial is not None:
        ...
        best_cfg_path = out_dir / "best_by_score_backtest_config.yaml"
    ...
    summary = {
        ...
        "best_by_score_config": (None if best_cfg_path is None else str(best_cfg_path)),
    ...
    if best_cfg_path is not None:
        print(f"- best_by_score_config: {best_cfg_path}")
