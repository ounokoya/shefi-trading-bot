...
from libs.optimizations.zerem_multi.runner import ZeremOptunaConfig, pareto_front_df, run_optuna_multi
...
def main() -> None:
    ...
    ap.add_argument(
        "--objective-mode",
        choices=["equity", "monthly"],
        default=str(cfg_dict.get("objective_mode", "equity")),
    )
    ap.add_argument(
        "--monthly-mad-weight",
        type=float,
    ...
    ap.add_argument("--db", default=str(cfg_dict.get("db", str(PROJECT_ROOT / "data" / "processed" / "optuna" / "zerem_multi.db"))))
    ap.add_argument(
        "--out-csv",
        default=str(cfg_dict.get("out_csv", str(PROJECT_ROOT / "data" / "processed" / "optuna" / "zerem_multi_pareto.csv"))),
    )
    ...
    if not str(args.study_name or "").strip():
        ...
        args.study_name = _slug(
            f"zerem_multi_{args.symbol}_{args.grid_year}_{tf_slug}_k{args.max_combo_size}_tfk{args.entry_cci_tf_max_combo_size}_{'ccitf' if args.entry_cci_tf_confluence else 'notf'}_{args.exit_b_mode}_{series_slug}_combok_exact"
        )
    ...
    try:
        df = pareto_front_df(study)
    except Exception as e:
        df = pd.DataFrame([])
        print(f"No best trials found (all trials may have been pruned). {e}")
    out_csv = Path(args.out_csv)
    out_csv.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(out_csv, index=False)

    print("\n=== PARETO FRONT (multi objective) ===")
    print(f"n_best_trials={len(study.best_trials)}")
    if not df.empty:
        sort_cols = [c for c in ["objective", "ratio_final", "equity"] if c in df.columns]
        df2 = df.sort_values(sort_cols, ascending=[False] * len(sort_cols)).head(20)
