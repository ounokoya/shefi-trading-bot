...
def _get_score_cfg(raw: dict[str, Any]) -> tuple[dict[str, float], dict[str, float]]:
    ...
    scales_raw = score.get("scales") or {}
    ...
    weights = {
        "pnl": _f(weights_raw, "pnl", 1.0),
        "equity_dd": _f(weights_raw, "equity_dd", 1.0),
        "trade_dd": _f(weights_raw, "trade_dd", 1.0),
        "duration_s_max": _f(weights_raw, "duration_s_max", 1.0),
        "winrate": _f(weights_raw, "winrate", 1.0),
    }
    scales = {
        "pnl": max(1e-12, _f(scales_raw, "pnl", 1.0)),
        "equity_dd": max(1e-12, _f(scales_raw, "equity_dd", 1.0)),
        "trade_dd": max(1e-12, _f(scales_raw, "trade_dd", 1.0)),
        "duration_s_max": max(1e-12, _f(scales_raw, "duration_s_max", 3600.0)),
        "winrate": max(1e-12, _f(scales_raw, "winrate", 1.0)),
    }
...
def main() -> int:
    ...
    if not isinstance(out_cfg, dict):
        out_cfg = {}
    out_dir = Path(str(out_cfg.get("out_dir") or "data/processed/backtests/optuna_macd_momentum_two_tf_weighted"))
    ...
    study_name = str(optuna_cfg.get("study_name") or "macd_momentum_two_tf_weighted")
    ...
    weights, scales = _get_score_cfg(raw)
    ...
    def _score(m: dict[str, float]) -> float:
        pnl = float(m["pnl"]) / float(scales["pnl"])
        eq_dd = float(m["equity_dd"]) / float(scales["equity_dd"])
        tr_dd = float(m["trade_dd"]) / float(scales["trade_dd"])
        dur = float(m["duration_s_max"]) / float(scales["duration_s_max"])
        wr = float(m["winrate"]) / float(scales["winrate"])

        return (
            float(weights["pnl"]) * float(pnl)
            - float(weights["equity_dd"]) * float(eq_dd)
            - float(weights["trade_dd"]) * float(tr_dd)
            - float(weights["duration_s_max"]) * float(dur)
            + float(weights["winrate"]) * float(wr)
        )
    ...
    print("Optuna MacdMomentumTwoTF weighted optimization:")
