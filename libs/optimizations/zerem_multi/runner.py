...
def _objective(trial: optuna.Trial, *, cfg: ZeremOptunaConfig) -> tuple[float, float, float]:
    ...
    objective_mode = str(getattr(cfg, "objective_mode", "equity") or "equity").strip().lower()
    monthly_ratio_sum = 0.0
    monthly_ratio_mad = 0.0
    monthly_mad_weight = 0.0
    objective_raw = float(total_score.equity)
    objective_base = float(total_score.equity)
    if objective_mode == "monthly":
        mo = getattr(cfg, "monthly_objective", None)
        if isinstance(mo, dict):
            monthly_mad_weight = float(mo.get("mad_weight", 0.0) or 0.0)
        monthly_ratio_sum, monthly_ratio_mad, objective_base = monthly_ratio_objective_from_report(
            month_df, mad_weight=float(monthly_mad_weight)
        )
        objective_raw = float(monthly_ratio_sum)
        trial.set_user_attr("monthly_ratio_sum", float(monthly_ratio_sum))
        trial.set_user_attr("monthly_ratio_mad", float(monthly_ratio_mad))
        trial.set_user_attr("monthly_mad_weight", float(monthly_mad_weight))
        trial.set_user_attr("objective_mode", "monthly")
    else:
        trial.set_user_attr("objective_mode", "equity")
    ...
    if isinstance(getattr(cfg, "regularity_penalty", None), dict) and bool(cfg.regularity_penalty.get("enabled", False)):
        ...
        weight_equity = float(cfg.regularity_penalty.get("weight_equity", 0.0) or 0.0)
    ...
    objective_adj = float(objective_base) - float(weight_equity) * float(penalty)

    if objective_mode == "equity":
        equity_adj = float(objective_adj)
    else:
        equity_adj = float(total_score.equity)
    ...
    trial.set_user_attr("ratio_final", float(total_score.ratio_final))
    ...
    trial.set_user_attr("regularity_weight_equity", float(weight_equity))
    if penalty_stats:
        for k, v in penalty_stats.items():
            trial.set_user_attr(f"regularity_{k}", float(v))
    trial.set_user_attr("equity_adj", float(equity_adj))

    return float(objective_adj), float(total_score.max_dd), float(total_score.worst_mae)


def run_optuna_multi(
    *,
    ...
    if bool(cfg.offline):
        ...
        cfg = ZeremOptunaConfig(
            ...
            cache_validate=bool(cfg.cache_validate),
            cache_max_missing=int(cfg.cache_max_missing),
            objective_mode=str(getattr(cfg, "objective_mode", "equity") or "equity"),
...
def pareto_front_df(study: optuna.Study) -> pd.DataFrame:
    rows: List[Dict] = []
    for tr in study.best_trials:
        vals = tr.values or (None, None, None)
        objective = float(vals[0]) if vals[0] is not None else 0.0
        max_dd = float(vals[1]) if vals[1] is not None else 0.0
        worst_mae = float(vals[2]) if vals[2] is not None else 0.0
        rows.append(
            {
                "trial": int(tr.number),
                "objective": float(objective),
                "objective_mode": str(tr.user_attrs.get("objective_mode", "")),
                "objective_raw": float(tr.user_attrs.get("objective_raw", float("nan"))),
                "objective_adj": float(tr.user_attrs.get("objective_adj", float("nan"))),
                "equity": float(tr.user_attrs.get("equity", float("nan"))),
                "equity_adj": float(tr.user_attrs.get("equity_adj", float("nan"))),
                "max_dd": float(max_dd),
                "worst_mae": float(worst_mae),
                "ratio_final": float(tr.user_attrs.get("ratio_final", float("nan"))),
                "n_trades": int(tr.user_attrs.get("n_trades", 0)),
                "winrate": float(tr.user_attrs.get("winrate", 0.0)),
                "regularity_penalty": float(tr.user_attrs.get("regularity_penalty", float("nan"))),
                **{f"p:{k}": v for k, v in (tr.params or {}).items()},
            }
        )
