...
class TripleVwmaCycleMetrics:
    ...
    zone_mid_radius_pct: float
    zone_slow_radius_pct: float

    macro_trend_side: str
    micro_trend_side: str

    spread_ms_max_pct: float | None
    spread_ms_end_pct: float | None
    spread_ms_slope_mean_pct: float | None

    spread_fm_max_pct: float | None
    spread_fm_end_pct: float | None
    spread_fm_slope_mean_pct: float | None

    vwma_fast_slope_mean_pct: float | None
    vwma_mid_slope_mean_pct: float | None
...
@dataclass(frozen=True)
class TripleVwmaCycleAgentConfig:
    ts_col: str = "ts"
    open_col: str = "open"
    high_col: str = "high"
    low_col: str = "low"
    ...
    vwma_fast_col: str = "vwma_12"
    vwma_mid_col: str = "vwma_72"
    vwma_slow_col: str = "vwma_168"

    zone_fast_radius_pct: float = 0.001
    zone_mid_radius_pct: float = 0.001
...
class TripleVwmaCycleAgent:
    def __init__(self, *, cfg: TripleVwmaCycleAgentConfig | None = None):
        self.cfg = cfg or TripleVwmaCycleAgentConfig()
    ...
    @staticmethod
    def _sep_state(*, a_upper: np.ndarray, a_lower: np.ndarray, b_upper: np.ndarray, b_lower: np.ndarray) -> np.ndarray:
        finite_mask = np.isfinite(a_upper) & np.isfinite(a_lower) & np.isfinite(b_upper) & np.isfinite(b_lower)
        sep = np.zeros(int(len(a_upper)), dtype=int)
        above = finite_mask & (a_lower > b_upper)
        below = finite_mask & (a_upper < b_lower)
        sep = np.where(above, 1, sep)
        sep = np.where(below, -1, sep)
        return sep

    def _segment_cycles(self, df: pd.DataFrame) -> list[tuple[int, int]]:
        cfg = self.cfg
        for c in (cfg.ts_col, cfg.vwma_mid_col, cfg.vwma_slow_col):
            if str(c) not in df.columns:
                raise ValueError(f"Missing required column: {c}")

        mid = pd.to_numeric(df[str(cfg.vwma_mid_col)], errors="coerce").astype(float).to_numpy()
        slow = pd.to_numeric(df[str(cfg.vwma_slow_col)], errors="coerce").astype(float).to_numpy()

        mu, ml = self._bounds(mid, float(cfg.zone_mid_radius_pct))
        su, sl = self._bounds(slow, float(cfg.zone_slow_radius_pct))

        sep_state = self._sep_state(a_upper=mu, a_lower=ml, b_upper=su, b_lower=sl)
    ...
    def _analyze_cycle(self, df: pd.DataFrame, *, cycle_id: int, start: int, end: int) -> TripleVwmaCycleMetrics | None:
        cfg = self.cfg
        ...
        mid = pd.to_numeric(w[str(cfg.vwma_mid_col)], errors="coerce").astype(float).to_numpy()
        ...
        zm = float(cfg.zone_mid_radius_pct)
        ...
        fu, fl = self._bounds(fast, zf)
        mu, ml = self._bounds(mid, zm)
        su, sl = self._bounds(slow, zs)

        fu_l, fl_l = self._bounds(fast, zf * zone_large_mult)
        mu_l, ml_l = self._bounds(mid, zm * zone_large_mult)
        su_l, sl_l = self._bounds(slow, zs * zone_large_mult)

        def _sep_sign_from_bounds(*, au: float, al: float, bu: float, bl: float) -> int:
            if float(al) > float(bu):
                return 1
            if float(au) < float(bl):
                return -1
            return 0

        macro_trend_side = "SHORT"
        ms0 = None
        try:
            au0 = _safe_float(mu[0])
            al0 = _safe_float(ml[0])
            bu0 = _safe_float(su[0])
            bl0 = _safe_float(sl[0])
            if au0 is not None and al0 is not None and bu0 is not None and bl0 is not None:
                ms0 = int(_sep_sign_from_bounds(au=float(au0), al=float(al0), bu=float(bu0), bl=float(bl0)))
                if ms0 == 1:
                    macro_trend_side = "LONG"
                elif ms0 == -1:
                    macro_trend_side = "SHORT"
                else:
                    m0 = _safe_float(mid[0])
                    s0 = _safe_float(slow[0])
                    macro_trend_side = "LONG" if (m0 is not None and s0 is not None and float(m0) > float(s0)) else "SHORT"
        except Exception:
            m0 = _safe_float(mid[0])
            s0 = _safe_float(slow[0])
            macro_trend_side = "LONG" if (m0 is not None and s0 is not None and float(m0) > float(s0)) else "SHORT"

        micro_trend_side = "SHORT"
        try:
            au0 = _safe_float(fu[0])
            al0 = _safe_float(fl[0])
            bu0 = _safe_float(mu[0])
            bl0 = _safe_float(ml[0])
            if au0 is not None and al0 is not None and bu0 is not None and bl0 is not None:
                fm0 = int(_sep_sign_from_bounds(au=float(au0), al=float(al0), bu=float(bu0), bl=float(bl0)))
                if fm0 == 1:
                    micro_trend_side = "LONG"
                elif fm0 == -1:
                    micro_trend_side = "SHORT"
                else:
                    f0 = _safe_float(fast[0])
                    m0 = _safe_float(mid[0])
                    micro_trend_side = "LONG" if (f0 is not None and m0 is not None and float(f0) > float(m0)) else "SHORT"
        except Exception:
            f0 = _safe_float(fast[0])
            m0 = _safe_float(mid[0])
            micro_trend_side = "LONG" if (f0 is not None and m0 is not None and float(f0) > float(m0)) else "SHORT"

        dir_sign = 1.0 if macro_trend_side == "LONG" else -1.0

        def _spread_stats(a: np.ndarray, b: np.ndarray, denom: np.ndarray) -> tuple[float | None, float | None, float | None]:
            if int(len(w)) == 0:
                return None, None, None
            den = np.where(np.isfinite(denom) & (denom != 0.0), denom, np.nan)
            sp = np.abs((a - b) / den)
            sp_ok = sp[np.isfinite(sp)]
            sp_max = float(np.nanmax(sp_ok)) if sp_ok.size else None
            sp_end = None
            d0 = _safe_float(denom[-1])
            if d0 is not None and d0 != 0.0:
                sp_end = float(abs(float(a[-1]) - float(b[-1])) / float(d0))
            sp_slope = None
            if int(len(w)) >= 3:
                den0 = np.where(np.isfinite(denom[:-1]) & (denom[:-1] != 0.0), denom[:-1], np.nan)
                den1 = np.where(np.isfinite(denom[1:]) & (denom[1:] != 0.0), denom[1:], np.nan)
                sp0 = np.abs((a[:-1] - b[:-1]) / den0)
                sp1 = np.abs((a[1:] - b[1:]) / den1)
                d = sp1 - sp0
                d = d[np.isfinite(d)]
                if d.size:
                    sp_slope = float(np.mean(d))
            return sp_max, sp_end, sp_slope

        spread_ms_max_pct, spread_ms_end_pct, spread_ms_slope_mean_pct = _spread_stats(mid, slow, slow)
        spread_fm_max_pct, spread_fm_end_pct, spread_fm_slope_mean_pct = _spread_stats(fast, mid, mid)

        vwma_fast_slope_mean_pct = None
        vwma_mid_slope_mean_pct = None
        vwma_slow_slope_mean_pct = None
        vwma_slope_harmony_ratio = None
        if int(len(w)) >= 3:
            def _slope_mean(x: np.ndarray) -> float | None:
                x0 = x[:-1]
                x1 = x[1:]
                den = np.where(np.isfinite(x0) & (x0 != 0.0), x0, np.nan)
                s = (x1 - x0) / den
                ok = s[np.isfinite(s)]
                return float(np.mean(ok)) if ok.size else None

            vwma_fast_slope_mean_pct = _slope_mean(fast)
            vwma_mid_slope_mean_pct = _slope_mean(mid)
            vwma_slow_slope_mean_pct = _slope_mean(slow)
            ...
            mden = np.where(np.isfinite(m0) & (m0 != 0.0), m0, np.nan)
            sden = np.where(np.isfinite(s0) & (s0 != 0.0), s0, np.nan)
            fs = (f1 - f0) / fden
            ms = (m1 - m0) / mden
            ss = (s1 - s0) / sden
            ok = (dir_sign * fs > 0.0) & (dir_sign * ms > 0.0) & (dir_sign * ss > 0.0)
            ok = ok[np.isfinite(fs) & np.isfinite(ms) & np.isfinite(ss)]
            if ok.size:
                vwma_slope_harmony_ratio = float(np.mean(ok.astype(float)))

        events: list[TripleVwmaEvent] = []
        ...
        def _sep_sign_from_values(fv: float, mv: float, sv: float) -> int | None:
            if (not math.isfinite(float(mv))) or (not math.isfinite(float(sv))):
                return None
            au = float(mv) * (1.0 + float(zm))
            al = float(mv) * (1.0 - float(zm))
            bu = float(sv) * (1.0 + float(zs))
            bl = float(sv) * (1.0 - float(zs))
            return int(_sep_sign_from_bounds(au=float(au), al=float(al), bu=float(bu), bl=float(bl)))

        _ev(
            "cycle_start",
            0,
            {
                "macro_trend_side": str(macro_trend_side),
                "micro_trend_side": str(micro_trend_side),
                "is_aligned": bool(str(macro_trend_side) == str(micro_trend_side)),
                "start_pos": 0,
                "end_pos": int(len(w) - 1),
            },
        )
        ...
        if int(start) > 0:
            ...
            try:
                pm = float(pd.to_numeric(prev[str(cfg.vwma_mid_col)], errors="coerce"))
                ps = float(pd.to_numeric(prev[str(cfg.vwma_slow_col)], errors="coerce"))
                cm = float(pd.to_numeric(w[str(cfg.vwma_mid_col)].iloc[0], errors="coerce"))
                cs = float(pd.to_numeric(w[str(cfg.vwma_slow_col)].iloc[0], errors="coerce"))
                prev_sep = _sep_sign_from_values(0.0, float(pm), float(ps))
                cur_sep = _sep_sign_from_values(0.0, float(cm), float(cs))
        ...
        def _emit_pair_collisions(*, pair: str, a: np.ndarray, b: np.ndarray, a_u: np.ndarray, a_l: np.ndarray, b_u: np.ndarray, b_l: np.ndarray) -> None:
            sep_state = self._sep_state(a_upper=a_u, a_lower=a_l, b_upper=b_u, b_lower=b_l)
            finite_mask = np.isfinite(a_u) & np.isfinite(a_l) & np.isfinite(b_u) & np.isfinite(b_l)
            is_collision = finite_mask & (sep_state == 0)

            i0 = 0
            while i0 < int(len(w)):
                if not bool(is_collision[i0]):
                    i0 += 1
                    continue

                j0 = int(i0)
                while j0 < int(len(w)) and bool(is_collision[j0]):
                    j0 += 1
                ...
                if int(end_pos) >= int(start_pos):
                    seg = np.abs(a[int(start_pos) : int(end_pos) + 1] - b[int(start_pos) : int(end_pos) + 1])
        ...
        _emit_pair_collisions(pair="mid_slow", a=mid, b=slow, a_u=mu, a_l=ml, b_u=su, b_l=sl)
        _emit_pair_collisions(pair="fast_mid", a=fast, b=mid, a_u=fu, a_l=fl, b_u=mu, b_l=ml)
        ...
        def _emit_zone_tests(
            *,
            ...
            for k in range(int(len(w))):
                ...
                is_contra = bool((macro_trend_side == "LONG" and direction == "DOWN") or (macro_trend_side == "SHORT" and direction == "UP"))
                _ev(
                    "zone_test",
                    int(end_pos),
                    {
        ...
        for zname, uu, ll, uu_l, ll_l, inner_uu, inner_ll in (
            ("fast", fu, fl, fu_l, fl_l, fu, fl),
            ("mid", mu, ml, mu_l, ml_l, mu, ml),
            ("slow", su, sl, su_l, sl_l, su, sl),
        ):
            _emit_zone_tests(zone=str(zname), layer="security", lower=ll, upper=uu)
            _emit_zone_tests(
                zone=str(zname),
                layer="large",
                lower=ll_l,
                upper=uu_l,
                inner_lower=inner_ll,
                inner_upper=inner_uu,
        ...
        def _on_trend_side(pos: int) -> bool:
            if macro_trend_side == "LONG":
                return bool(_safe_float(l[pos]) is not None and _safe_float(fu[pos]) is not None and float(l[pos]) >= float(fu[pos]))
            return bool(_safe_float(h[pos]) is not None and _safe_float(fl[pos]) is not None and float(h[pos]) <= float(fl[pos]))
        ...
        for i in range(int(len(w))):
            fu_i = _safe_float(fu[i])
            fl_i = _safe_float(fl[i])
            mu_i = _safe_float(mu[i])
            ml_i = _safe_float(ml[i])
            su_i = _safe_float(su[i])
            sl_i = _safe_float(sl[i])
            if fu_i is None or fl_i is None or mu_i is None or ml_i is None or su_i is None or sl_i is None:
                continue

            on_trend = _on_trend_side(int(i))

            touched_fast = _touch_zone(int(i), upper=float(fu_i), lower=float(fl_i))
            touched_mid = _touch_zone(int(i), upper=float(mu_i), lower=float(ml_i))
            touched_slow = _touch_zone(int(i), upper=float(su_i), lower=float(sl_i))
            ...
            if hi is not None and li is not None:
                if macro_trend_side == "LONG":
                    broke_fast = bool(float(li) < float(fl_i))
                    broke_mid = bool(float(li) < float(ml_i))
                    broke_slow = bool(float(li) < float(sl_i))
                else:
                    broke_fast = bool(float(hi) > float(fu_i))
                    broke_mid = bool(float(hi) > float(mu_i))
                    broke_slow = bool(float(hi) > float(su_i))

            trigger = bool(touched_fast or broke_fast or touched_mid or broke_mid or touched_slow or broke_slow)
            if (not in_pullback) and (not on_trend) and bool(trigger):
                in_pullback = True
                pb_start = int(i)
                pb_stage = 0
                pb_break_confirmed = False
            ...
            if in_pullback:
                ...
                if touched_fast or broke_fast:
                    pb_stage = max(int(pb_stage), 1)
                if touched_mid or broke_mid:
                    pb_stage = max(int(pb_stage), 2)
                if touched_slow or broke_slow:
                    pb_stage = max(int(pb_stage), 3)
                ...
                if (not reversal_confirmed) and pb_extreme_pos is not None and int(i) > int(pb_extreme_pos):
                    ...
                    if oi is None or ci is None:
                        ...
                        candle_ok = bool((float(ci) > float(oi))) if macro_trend_side == "LONG" else bool((float(ci) < float(oi)))
                        if candle_ok:
                            reversal_run += 1
                        else:
                            reversal_run = 0
                    ...
                    if reversal_run >= int(reversal_confirm_bars):
                        ...
                        if bool(cfg.reversal_impact_filter_enabled):
                            target_color = "GREEN" if str(macro_trend_side) == "LONG" else "RED"
                        ...
                        if bool(cfg.micro_filter_enabled):
                            ...
                            if str(z2.get("side")) != str(macro_trend_side):
                                continue
                            meta["micro"] = dict(z2)
                    ...
                    if slow_confirm_run >= int(break_confirm_bars):
                        ...
                        _ev(
                            ...
                            int(i),
                            {
                                ...
                                "broke_mid": bool(broke_mid),
                                "broke_slow": bool(broke_slow),
                                "bar_high": float(hi) if hi is not None else None,
                                "bar_low": float(li) if li is not None else None,
                                "fast_upper": float(fu_i),
                                "fast_lower": float(fl_i),
                                "mid_upper": float(mu_i),
                                "mid_lower": float(ml_i),
                                "slow_upper": float(su_i),
                                "slow_lower": float(sl_i),
                            },
                        )
                ...
                if on_trend:
                    ...
                    _ev(
                        ...
                        int(pb_end),
                        {
                            ...
                            "stage": int(pb_stage),
                            "extreme_pos": (None if pb_extreme_pos is None else int(pb_extreme_pos)),
                            "extreme_price": (None if pb_extreme_price is None else float(pb_extreme_price)),
                        },
                    )
        ...
        if spread_ref <= 0:
            spread_ref = 0.002
        spread_factor = float(min(1.0, float(max((spread_ms_max_pct or 0.0), (spread_fm_max_pct or 0.0))) / spread_ref))
        ...
        events_sorted = sorted(events, key=lambda e: (int(e.pos), int(e.ts), str(e.kind)))
        events_out = [asdict(e) for e in events_sorted]

        return TripleVwmaCycleMetrics(
            ...
            vwma_mid_col=str(cfg.vwma_mid_col),
            vwma_slow_col=str(cfg.vwma_slow_col),
            zone_fast_radius_pct=float(zf),
            zone_mid_radius_pct=float(zm),
            zone_slow_radius_pct=float(zs),
            macro_trend_side=str(macro_trend_side),
            micro_trend_side=str(micro_trend_side),
            spread_ms_max_pct=(None if spread_ms_max_pct is None else float(spread_ms_max_pct)),
            spread_ms_end_pct=(None if spread_ms_end_pct is None else float(spread_ms_end_pct)),
            spread_ms_slope_mean_pct=(None if spread_ms_slope_mean_pct is None else float(spread_ms_slope_mean_pct)),
            spread_fm_max_pct=(None if spread_fm_max_pct is None else float(spread_fm_max_pct)),
            spread_fm_end_pct=(None if spread_fm_end_pct is None else float(spread_fm_end_pct)),
            spread_fm_slope_mean_pct=(None if spread_fm_slope_mean_pct is None else float(spread_fm_slope_mean_pct)),
            vwma_fast_slope_mean_pct=(None if vwma_fast_slope_mean_pct is None else float(vwma_fast_slope_mean_pct)),
            vwma_mid_slope_mean_pct=(None if vwma_mid_slope_mean_pct is None else float(vwma_mid_slope_mean_pct)),
    ...
    def analyze_df(self, df: pd.DataFrame, *, max_cycles: int = 0) -> list[TripleVwmaCycleMetrics]:
        ...
        out: list[TripleVwmaCycleMetrics] = []
    ...
    def current_df(self, df: pd.DataFrame) -> TripleVwmaCycleMetrics | None:
        cycles = self._segment_cycles(df)
        if not cycles:
            return None
    ...
    def answer(self, *, question: dict[str, Any], df: pd.DataFrame) -> dict[str, Any]:
        ...
        if kind in {"analyze", "analyze_triple_vwma_cycles"}:
        ...
        if kind in {"current", "current_triple_vwma_cycle"}:
        ...
        if kind in {"rank_triple_vwma_cycles", "rank", ""}:
