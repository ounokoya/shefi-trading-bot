...
@dataclass(frozen=True)
class TripleStochGlobalMetrics:
    pos: int
    ts: int
    dt: str
...
class TripleStochGlobalAgent:
    def __init__(self, *, cfg: TripleStochGlobalAgentConfig | None = None):
        self.cfg = cfg or TripleStochGlobalAgentConfig()
    ...
    def enrich_df(self, df: pd.DataFrame, *, in_place: bool = False) -> pd.DataFrame:
        ...
        need = {
            ...
            str(cfg.k_fast_col),
            str(cfg.d_fast_col),
            str(cfg.k_medium_col),
            str(cfg.d_medium_col),
            str(cfg.k_slow_col),
            str(cfg.d_slow_col),
        }
        ...
        k_med = pd.to_numeric(df[str(cfg.k_medium_col)], errors="coerce").astype(float).to_numpy()
        d_med = pd.to_numeric(df[str(cfg.d_medium_col)], errors="coerce").astype(float).to_numpy()
        ...
        ok_k = np.isfinite(k_fast) & np.isfinite(k_med) & np.isfinite(k_slow)
        ok_d = np.isfinite(d_fast) & np.isfinite(d_med) & np.isfinite(d_slow)

        k_global = np.full(int(len(df)), np.nan, dtype=float)
        d_global = np.full(int(len(df)), np.nan, dtype=float)

        k_global[ok_k] = (k_fast[ok_k] * float(w_fast) + k_med[ok_k] * float(w_med) + k_slow[ok_k] * float(w_slow)) / float(
            w_sum
        )
        d_global[ok_d] = (d_fast[ok_d] * float(w_fast) + d_med[ok_d] * float(w_med) + d_slow[ok_d] * float(w_slow)) / float(
            w_sum
        )
    ...
    def current_metrics(self, df: pd.DataFrame) -> TripleStochGlobalMetrics | None:
        cfg = self.cfg
        ...
        return TripleStochGlobalMetrics(
            pos=int(i),
            ts=int(ts_i),
            dt=str(dt),
            ...
            weight_medium=float(w_med),
            weight_slow=float(w_slow),
            weight_sum=float(w_sum),
        )

    def find_cross_events(self, df: pd.DataFrame, *, max_events: int = 200) -> list[TripleStochGlobalEvent]:
        cfg = self.cfg
        work = self.enrich_df(df, in_place=False)

        ts_s = pd.to_numeric(work[str(cfg.ts_col)], errors="coerce").astype("Int64").to_numpy()

        out: list[TripleStochGlobalEvent] = []
