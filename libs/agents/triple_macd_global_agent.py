...
@dataclass(frozen=True)
class TripleMacdGlobalAgentConfig:
    ts_col: str = "ts"
    dt_col: str = "dt"

    hist_fast_col: str = "macd_hist_fast"
    hist_medium_col: str = "macd_hist_medium"
    hist_slow_col: str = "macd_hist_slow"

    slow_period_fast: int = 26
    slow_period_medium: int = 42
...
class TripleMacdGlobalAgent:
    def __init__(self, *, cfg: TripleMacdGlobalAgentConfig | None = None):
        self.cfg = cfg or TripleMacdGlobalAgentConfig()
    ...
    def _weights(self) -> tuple[float, float, float, float] | None:
        ...
        l_med = int(cfg.slow_period_medium)
        l_slow = int(cfg.slow_period_slow)
        if l_fast <= 0 or l_med <= 0 or l_slow <= 0:
            return None

        w_fast = 1.0
        w_med = float(l_med) / float(l_fast)
    ...
    def enrich_df(self, df: pd.DataFrame, *, in_place: bool = False) -> pd.DataFrame:
        cfg = self.cfg
        need = {str(cfg.ts_col), str(cfg.hist_fast_col), str(cfg.hist_medium_col), str(cfg.hist_slow_col)}
        ...
        h_med = pd.to_numeric(df[str(cfg.hist_medium_col)], errors="coerce").astype(float).to_numpy()
        h_slow = pd.to_numeric(df[str(cfg.hist_slow_col)], errors="coerce").astype(float).to_numpy()

        ok = np.isfinite(h_fast) & np.isfinite(h_med) & np.isfinite(h_slow)
        h_global = np.full(int(len(df)), np.nan, dtype=float)
        h_global[ok] = (h_fast[ok] * float(w_fast) + h_med[ok] * float(w_med) + h_slow[ok] * float(w_slow)) / float(
            w_sum
        )
    ...
    def current_metrics(self, df: pd.DataFrame) -> TripleMacdGlobalMetrics | None:
        cfg = self.cfg
        ...
        return TripleMacdGlobalMetrics(
            pos=int(i),
            ts=int(ts_i),
            dt=str(dt),
    ...
    def find_zero_cross_events(self, df: pd.DataFrame, *, max_events: int = 200) -> list[TripleMacdGlobalEvent]:
        cfg = self.cfg
        work = self.enrich_df(df, in_place=False)
        ts_s = pd.to_numeric(work[str(cfg.ts_col)], errors="coerce").astype("Int64").to_numpy()

        out: list[TripleMacdGlobalEvent] = []
