...
@dataclass(frozen=True)
class DoubleCciTrancheAgentConfig:
    ts_col: str = "ts"
    high_col: str = "high"
    low_col: str = "low"
    close_col: str = "close"
...
class DoubleCciTrancheAgent:
    def __init__(self, *, cfg: DoubleCciTrancheAgentConfig | None = None):
        self.cfg = cfg or DoubleCciTrancheAgentConfig()
    ...
    def _global_cci(self, tranche: pd.DataFrame) -> tuple[np.ndarray, tuple[float, float, float] | None]:
        cfg = self.cfg
        w = self._weights()
        if w is None:
        ...
        g[ok] = (fast[ok] * float(w_fast) + slow[ok] * float(w_slow)) / float(w_sum)
        return g, w
    ...
    def _to_metrics(self, tranche: pd.DataFrame) -> DoubleCciTrancheMetrics | None:
        cfg = self.cfg
        if len(tranche) == 0:
            return None

        need = {cfg.ts_col, cfg.cci_fast_col, cfg.cci_slow_col}
        ...
        return DoubleCciTrancheMetrics(
            tranche_id=int(tid),
    ...
    def analyze_df(self, df: pd.DataFrame, *, max_tranches: int = 0) -> list[DoubleCciTrancheMetrics]:
        cfg = self.cfg
        ...
        out: list[DoubleCciTrancheMetrics] = []
        for tid in selected:
    ...
    def current_df(self, df: pd.DataFrame) -> DoubleCciTrancheMetrics | None:
        cfg = self.cfg
        work = segment_macd_hist_tranches_df(
            df,
            ts_col=str(cfg.ts_col),
    ...
    def answer(self, *, question: dict[str, Any], df: pd.DataFrame) -> dict[str, Any]:
        ...
        if kind in {"", "analyze", "analyze_cci_tranches", "classify_cci_tranches"}:
        ...
        if kind in {"current", "current_cci_tranche"}:
