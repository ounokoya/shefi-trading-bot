...
class CCI4hTf15mBrain(BaseBrain):
    """
    CCI 4H Brain (on 15m Data):
    - Resamples 5m source data to 15m.
    - Calculates CCI(16) on 15m data (16 * 15m = 4 Hours).
    - Optional Klinger Filter (on 15m data).
    """
    def __init__(self, source_df: pd.DataFrame, source_tf: str = "5m", filter_mode: str = "none"):
        ...
        # 2. Compute CCI(16) on 15m
        self.cci_series_15m = self._compute_cci(self.df_15m, length=16)
        
        # 3. Compute Klinger on 15m if needed
        if self.filter_mode in ['kvo', 'signal']:
            self._compute_klinger_15m()
            
        # 4. Align back to source (5m) indices
        self.cci_map = self._create_map(self.source_df, self.cci_series_15m)
    ...
    def get_intentions(self, timestamp: int, account_state: AccountState) -> List[SignalIntent]:
        cci_val = self.cci_map.get(timestamp, 0.0)
        
        if pd.isna(cci_val): return []
